...
var StratumClient = function(options){
    ...
    function handleSubmit(message){ // Пулу на подтверждение приходит шара
        ...
        if (!_this.extraNonce1){ // Для данного пользователя отсутствует extranonce1. Проверка невозможна.
            sendJson({ 
                id    : message.id,
                result: null,
        ...
        _this.emit('submit', 
            {
                nonce     : message.params[0], // extranonce + miningmonce
                header    : message.params[1], // транзакции, информация и пр.
                mix       : message.params[2] // получившийся результат
