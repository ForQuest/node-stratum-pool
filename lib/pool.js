...
var pool = module.exports = function pool(options, authorizeFn) {
    ...
    function GetFirstJob(finishedCallback) {
        ...
        GetWork(function (error, result) {
            ...
            var networkDiffAdjusted = options.difficulty;
    ...
    function OutputPoolInfo() {
        ...
        var infoLines = [startMessage,
            ...
            'Total Difficulty:\t' + options.totalDifficulty,
            'Network Hash Rate:\t' + util.getReadableHashRateString(options.difficulty/25),
            'Stratum Port(s):\t' + _this.options.stratumPorts.join(', '),
    ...
    function SetupJobManager(callback) { // Инициализирует экземпляр класса jobManager, методы которого служат для запуска обработки новой шары, блока
        // и обновления блока.
        ...
        _this.jobManager.on('newBlock', function (blockTemplate) {
            ...
            if (!isValidBlock)
                ...
                if (jobManagerLastSubmitBlockHex === shareData.mixDigest) {
                    emitWarningLog('Warning, ignored duplicate submit block mix' + shareData.mixDigest);
                } else {
                    jobManagerLastSubmitBlockHex = shareData.mixDigest;
    ...
    function StartStratumServer(finishedCallback) { // Запускаем стратум-сервер.
        ...
        _this.stratumServer.on('started', function () {
            ...
            options.stratumPorts = Object.keys(options.ports);
            ...
            client.on('difficultyChanged', function (diff) { // Ловим сигнал 'difficultyChanged' и генерим 'difficultyUpdate'.
                ...
                jobParams[3] = util.calcDifficulty(options.ports[client.socket.localPort].diff);
                this.sendDifficulty(jobParams[3]);
                ...
                _this.jobManager.processShare(
                    ...
                    function(result){
                        resultCallback(result.error, result.share);
                    }
                );
            ...
            }).on('hashrate', function (data) {
                _this.emit('sub_hashrate', data);
            });
        });
    }
            ...
            _this.daemon.cmd('eth_getWork',
                ...
                function (result) {
                    if (result.error) {
                        ...
                        response.height = bignum(blockTemplate.number.slice(2), 16).toNumber();
                        response.difficulty = bignum(blockTemplate.difficulty.slice(2), 16);
