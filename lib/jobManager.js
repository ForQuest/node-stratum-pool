...
var JobManager = module.exports = function JobManager(options) {
    ...
    this.processTemplate = function (rpcData) {
        ...
        if (typeof(_this.currentJob) === 'undefined') _this.ethash.init(rpcData.height);
        else {
            if (Math.floor(_this.currentJob.height/30000) < Math.floor(rpcData.height/30000)) {
                _this.DAGLoaded = false;
                _this.ethash.updateEpoc(rpcData.height);
            }
        }
    ...
    this.processShare = function (workerName, jobId, nonce, header, mix, difficulty, ipAddress, port, callback) {
        ...
        _this.ethash.submitShare(block, difficulty.slice(2), job.target.toString(16), function(result){
