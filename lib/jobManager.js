...
var ExtraNonceCounter = function (configInstanceId) { // Либо переданное числового значения, либо 
    // псевдослучайно сгенерированное 32-х битное число сдвигается на 27 бит. Это значение и будет extranonce. 
...
var JobManager = module.exports = function JobManager(options) {
    ...
    this.processShare = function (workerName, jobId, nonce, header, mix, difficulty, ipAddress, port) {
        ...
        var isValid = hasher.Verify(block, difficulty, job.target);

        _this.emit('share', isValid, {
            job: jobId,
            ip: ipAddress,
            port: port,
            worker: workerName,
            ...
            blockDiff: job.rpc.difficulty
        });

        return {result: isValid.share, error: null};
    };
};
